    ...
    $(function() {
        ...
        ui.slider = function() {
            $('.ui_slider').each(function() {
                ...
                if (options.pager == 'thumbnails' && $('.ui_slider').find('li').length > 1 ) {
                    thumbnails = true;
                    options.pager = false;

                    // Sync slider with thumbs
                    options.onSlideBefore = function($slide, from, to) {
                        ...
                        // Slide to active thumb
                        var position = $thumb.position().left;
                        if (position < 0 || position >= $slider.width())
                            $thumbs.trigger('slideTo', $thumb);
                    };
                }
                ...
                if (thumbnails) {
                    var $thumbs = $slider
                        ...
                        .end()
                        .end()
                        .wrap('<div class="ui_slider_thumbnails" />')
                        .parent()
                        .append('<div class="prev" />')
                        .append('<div class="next" />')
                        .insertAfter($slider)

                        // Disable autoslide when thumbs are hovered
                        .on({
                            mouseenter: function() {
                                slider.stopAuto();
                            },
                        ...
                        .on('click', 'li', function() {
                            var $thumb = $(this);
                            if ($thumb.parent().is('.active'))
                                return;
                            slider.goToSlide($thumb.data('id'));
                        });
                    
                    // Refresh thumbs height
                    function refresh() {
                        $thumbs
                            .add($thumbs.parent())
                            .height($('li:eq(0)', $thumbs).outerHeight());
                    }
                    utils.resize(refresh);
                    setInterval(refresh, 500);
                }

                // Init slider
                var slider = $('ul', $slider).bxSlider(options);
            });
        };
        ...
        common.menu = function() {
            ...
            if (utils.iDevice()) {
                ...
                $document.on('vclick', function(event) {
                    ...
                    if( $item.parents('#menu').length > 0 && $item.attr('href') != undefined ){
                    	document.location= $item. attr('href') ;
                    }
                    var go = !($item.parent().is('.submenu') || $item.parent().is('.current-menu-item'));
